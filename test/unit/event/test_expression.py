...
from ftrack_api.event.expression import Expression, All, Any, Not, Condition, Parser
...
def candidate():
    ...
    return {"id": 10, "name": "value", "change": {"name": "value", "new_value": 10}}
...
@pytest.mark.parametrize(
    ...
    [
        ...
        pytest.param(
            "key=value", Condition("key", operator.eq, "value"), id="Basic Condition"
        ),
        pytest.param(
            'key="value"',
            Condition("key", operator.eq, "value"),
            id="Basic Quoted Condition",
        ),
        pytest.param(
                ...
                [
                    ...
                    All(
                        [
                            ...
                            Not(Condition("g.h", operator.gt, 10)),
                        ]
                    ),
                ]
            ),
...
@pytest.mark.parametrize(
    ...
    [
        ...
        pytest.param(
            Condition("key", "=", "value"), "<Condition key=value>", id="Condition"
        ),
    ],
)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        pytest.param(
            All([Expression(), Condition("test", operator.eq, "value")]),
            False,
            id="All-not all match",
        ),
        # Any
        pytest.param(Any(), False, id="Any-no expressions never matches"),
        pytest.param(
            Any([Expression(), Condition("test", operator.eq, "value")]),
            True,
            id="Any-some match",
        ),
        pytest.param(
        ...
        pytest.param(
            Not(Not(Expression())), True, id="Not-double negative is positive match"
        ),
    ],
)
...
def parametrize_test_condition_match(metafunc):
    ...
    for operator_function, values in matrix.items():
        for value_label, value in values.items():
            if value_label.startswith("wildcard"):
                key_options = {"plain": "name", "nested": "change.name"}
            else:
                key_options = {"plain": "id", "nested": "change.new_value"}
    ...
    metafunc.parametrize("key, operator, value, expected", data)
