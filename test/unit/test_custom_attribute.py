...
def new_entity_and_custom_attribute(request, session):
    ...
    if request.param == "AssetVersion":
        entity = session.create(
            request.param, {"asset": session.query("Asset").first()}
        )
        ...
        entity = session.create(
            request.param, {"context_id": shot["project_id"], "name": str(uuid.uuid1())}
        )
...
def test_read_set_custom_attribute(
    ...
    custom_attribute_value = session.query(
        ...
        "{custom_attribute_name}".format(custom_attribute_name=custom_attribute_name)
    ).first()
...
def test_write_set_custom_attribute_value(session, entity_type, custom_attribute_name):
    ...
    entity = session.query(
        ...
        "custom_attributes.configuration.key is {custom_attribute_name}".format(
            entity_type=entity_type, custom_attribute_name=custom_attribute_name
        )
    ).first()
...
def test_read_custom_attribute_that_does_not_exist(
    ...
    entity = session.query(
        "select custom_attributes from {entity_type}".format(entity_type=entity_type)
    ).first()
...
def test_set_custom_attribute_on_new_but_persisted_version(session, new_asset_version):
...
@pytest.mark.xfail(
    raises=ftrack_api.exception.ServerError, reason="Due to user permission errors."
)
def test_batch_create_entity_and_custom_attributes(new_entity_and_custom_attribute):
...
def test_refresh_custom_attribute(new_asset_version):
    ...
    query_string = (
        'select custom_attributes from AssetVersion where id is "{0}"'.format(
    ...
    asset_version_two = session_two.query(query_string).first()
