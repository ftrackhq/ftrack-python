...
def new_container_component():
    ...
    entity = session.create("ContainerComponent", {"name": "container_component"})
...
def new_file_component(name="foo", container=None):
    ...
    entity = session.create(
        "FileComponent", {"name": name, "file_type": ".png", "container": container}
    )
...
@pytest.mark.parametrize(
    ...
    [
        ...
        pytest.param(
            ...
            "my_new_asset",
            id="file_component_on_project_with_prefix",
        ),
        pytest.param(
            file_component,
            ["baz1", "bar"],
        ...
        pytest.param(
            ...
            ftrack_api.structure.standard.StandardStructure(),
            "my_new_asset",
            id="slugify_non_ascii_hierarchy",
        ),
        pytest.param(
            file_component,
            ["björn!"],
        ...
        pytest.param(
            ...
            ftrack_api.structure.standard.StandardStructure(),
            "åäö",
            id="slugify_non_ascii_asset_name",
        ),
        pytest.param(
            file_component,
            [],
            ...
            ftrack_api.structure.standard.StandardStructure(),
            "my_ne!!!!w_asset",
            id="slugify_illegal_asset_name",
        ),
        pytest.param(
            file_component,
            ["björn2"],
            "{project_name}/björn2/my_new_asset/v001/foo.png",
            ftrack_api.structure.standard.StandardStructure(
...
def test_get_resource_identifier(
    ...
    for name in hierarchy:
        context_id = session.create(
            "Folder",
            {"name": name, "project_id": new_project["id"], "parent_id": context_id},
        )["id"]

    asset = session.create("Asset", {"name": asset_name, "context_id": context_id})
...
def test_component_without_version_relation(new_project):
    ...
    asset = session.create("Asset", {"name": "foo", "context_id": new_project["id"]})
    version = session.create("AssetVersion", {"asset": asset})
