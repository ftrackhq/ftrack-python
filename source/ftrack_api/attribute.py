    ...
    def get_value(attribute, entity):
        ...
        if attribute.name not in entity._inflated:
            ...
            if isinstance(
                ...
                logger.debug(
                    'Merging local value for attribute {0}.'.format(
                        attribute
                    )
                )
...
class Attribute(object):
    ...
    def is_set(self, entity):
        ...
        return any(
            [
                self.get_local_value(entity)
                is not ftrack_api.symbol.NOT_SET,
                self.get_remote_value(entity)
                is not ftrack_api.symbol.NOT_SET,
            ]
        )
    ...
    def populate_remote_value(self, entity):
        ...
        if default_projections:
            for projection in default_projections:
                projections.append(
                    '{0}.{1}'.format(self.name, projection)
                )
        else:
...
class AbstractCollectionAttribute(Attribute):
    ...
    def set_local_value(self, entity, value):
        ...
        super(AbstractCollectionAttribute, self).set_local_value(
            entity, value
        )
...
class KeyValueMappedCollectionAttribute(AbstractCollectionAttribute):
    ...
    def __init__(
        ...
        *value_attribute* should be the name of the attribute on an entity in
        ...
        super(KeyValueMappedCollectionAttribute, self).__init__(
            name, **kw
        )
    ...
    def _adapt_to_collection(self, entity, value):
        ...
        if not isinstance(
            ...
            if value is None:
                ...
                value, (list, ftrack_api.collection.Collection)
            ):
                ...
                expired_keys = set(current_value.keys()) - set(
                    value.keys()
                )
