...
class ConfigureCentralizedStorageScenario(object):
    ...
    def _get_confirmation_text(self, configuration):
        ...
        if configure_location:
            ...
            location = self.session.get('Location', select_location['location_id'])
    ...
    def configure_scenario(self, event):
        ...
        if previous_step == 'select_location':
            ...
            if values.get('location_id') != 'create_new_location':
                location_exists = self.session.query(
                    'Location where id is "{0}"'.format(values.get('location_id'))
                ).first()
        ...
        if previous_step == 'configure_location':
            ...
            if configure_location:
                ...
                if (
                    ...
                    next_step = 'configure_location'
                    warning_message += (
                        '**Location name, label and description cannot ' 'be empty.**'
                    )
                    values = configuration['select_location']
        ...
        if next_step == 'confirm_summary':
            items = [
                {'type': 'label', 'value': self._get_confirmation_text(configuration)}
            ]
            state = 'confirm'
            ...
            if select_location['location_id'] == 'create_new_location':
                ...
                location = self.session.create(
                    'Location',
                    {
                        ...
                        'description': (configure_location['location_description']),
                    },
                )

            else:
                location = self.session.query(
                    'Location where id is "{0}"'.format(select_location['location_id'])
                ).one()
...
class ActivateCentralizedStorageScenario(object):
    ...
    def activate(self, event):
        ...
        try:
            ...
            self.logger.info(
                L(
                    u'Storage scenario activated. Configured {0!r} from ' u'{1!r}',
                    location,
                    storage_scenario,
                )
            )
    ...
    def register(self, session):
        ...
        session.event_hub.subscribe(
            (
                ...
                'and data.storage_scenario.scenario="{0}"'.format(scenario_name)
            ),
            self.activate,
        )
