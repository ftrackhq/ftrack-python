...
class Factory(object):
    ...
    def create_reference_attribute(self, class_name, name, mutable, reference):
        ...
        return ftrack_api.attribute.ReferenceAttribute(name, reference, mutable=mutable)
    ...
    def create_mapped_collection_attribute(self, class_name, name, mutable, reference):
        ...
        if reference == "Metadata":
            ...
            def create_metadata(proxy, data, reference):
                ...
                data.update(
                    {"parent_id": entity["id"], "parent_type": entity.entity_type}
                )
            ...
            def creator(proxy, data):
                ...
                """
                entity = proxy.collection.entity
                if ftrack_api.inspection.state(entity) is not ftrack_api.symbol.CREATED:
                    raise KeyError(
                ...
                if configuration is None:
                    raise ValueError(
                        "No valid custom attribute for data {0!r} was found.".format(
                            data
                        )
                    )
                ...
                item = session.create(reference, create_data, reconstructing=True)
