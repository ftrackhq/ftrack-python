...
class Session(object):
    ...
    def create(self, entity_type, data=None, reconstructing=False):
        ...
        '''
        entity = self._create(
            entity_type, data, reconstructing=reconstructing
        )
    ...
    def _create(self, entity_type, data, reconstructing):
        ...
        try:
            ...
            raise ftrack_api.exception.UnrecognisedEntityTypeError(
                entity_type
            )

        return EntityTypeClass(
            self, data=data, reconstructing=reconstructing
        )
    ...
    def ensure(self, entity_type, data, identifying_keys=None):
        ...
        *data* is missing an identifying key then raise :exc:`KeyError`.
        ...
        try:
            ...
            self.logger.debug(
                'Creating entity as did not already exist.'
            )
    ...
    def get(self, entity_type, entity_key):
        ...
        '''
        self.logger.debug(
            L('Get {0} with key {1}', entity_type, entity_key)
        )

        primary_key_definition = self.types[
            entity_type
    ...
    def _merge_recursive(self, entity, merged=None):
        ...
        for attribute in entity.attributes:
            ...
            if isinstance(
                ...
                log_debug and self.logger.debug(
                    'Merging remote value for attribute {0}.'.format(
                        attribute
                    )
                )

                if isinstance(
                    remote_value, ftrack_api.entity.base.Entity
                ):
    ...
    def _merge_entity(self, entity, merged=None):
        ...
        *entity* will never be modified in place. Ensure that the returned
        ...
        with self.auto_populating(False):
            ...
            log_debug and self.logger.debug(
                'Checking for entity in cache with key {0}'.format(
                    entity_key
                )
            )
            try:
    ...
    def populate(self, entities, projections):
        ...
        self.logger.debug(
            L(
                'Populate {0!r} projections for {1}.',
                projections,
                entities,
            )
        )
        ...
        if entities_to_process:
            ...
            primary_key_definition = (
                reference_entity.primary_key_attributes
            )
            entity_keys = [
            ...
            if len(primary_key_definition) > 1:
                ...
                for entity_key in entity_keys:
                    ...
                    for key, value in zip(
                        primary_key_definition, entity_key
                    ):
                        condition.append(
                            '{0} is "{1}"'.format(key, value)
                        )

                    conditions.append(
                        '({0})'.format('and '.join(condition))
                    )

                query = '{0} where {1}'.format(
    ...
    def commit(self):
        ...
        for payload in batch:
            if (
                ...
                and previous_payload['entity_type']
                == payload['entity_type']
                and previous_payload['entity_key']
                == payload['entity_key']
            ):
                previous_payload['entity_data'].update(
                    payload['entity_data']
                )
                continue
                ...
                with self.operation_recording(False):
                    for entity in list(self._local_cache.values()):
                        for attribute in entity:
                            if (
                                attribute
                                not in entity.primary_key_attributes
                            ):
    ...
    def _load_schemas(self, schema_cache_path):
        ...
        if schema_cache_path:
            try:
                (
                    schemas,
                    local_schema_hash,
                ) = self._read_schemas_from_cache(schema_cache_path)
        ...
        if local_schema_hash != server_hash:
            ...
            if schema_cache_path:
                try:
                    self._write_schemas_to_cache(
                        schemas, schema_cache_path
                    )
    ...
    def _configure_locations(self):
        ...
        storage_scenario = self.server_information.get(
            'storage_scenario'
        )
    ...
    def call(self, data):
        ...
        data = self.encode(
            data, entity_attribute_strategy='modified_only'
        )
    ...
    def _encode(self, item, entity_attribute_strategy='set_only'):
        ...
        *entity_attribute_strategy* specifies how entity attributes should be
        ...
        '''
        if isinstance(
            item, (arrow.Arrow, datetime.datetime, datetime.date)
        ):
                ...
                for attribute in item.attributes:
                    ...
                    if value is not ftrack_api.symbol.NOT_SET:
                        if isinstance(
                            ...
                            if isinstance(
                                value, ftrack_api.entity.base.Entity
                            ):
    ...
    def create_component(self, path, data=None, location='auto'):
        ...
        *data* should be a dictionary of any additional data to construct the
        ...
        try:
            ...
            data.setdefault(
                'file_type', os.path.splitext(container_path)[-1]
            )
            ...
            for member_path in collection:
                member_data = {
                    'name': collection.match(member_path).group(
                        'index'
                    ),
                    'container': container,
    ...
    def _create_component(self, entity_type, path, data, location):
        ...
        if location:
            location.add_component(
                component, origin_location, recursive=False
            )
        ...
        for component in components:
            ...
            if is_container and len(component['members']):
                ...
                for member, member_availability in zip(
                    ...
                    for location_id, ratio in list(
                        member_availability.items()
                    ):
                        availability[location_id] += ratio * multiplier
            else:
                for component_location in component[
                    'component_locations'
                ]:
            ...
            for location_id, percentage in list(availability.items()):
                ...
                adjusted_percentage = max(
                    0.0, min(adjusted_percentage, 100.0)
                )
    ...
    def encode_media(
        self, media, version_id=None, keep_original='auto'
    ):
        ...
        *media* can be a path to a file or a FileComponent in the ftrack.server
        ...
        if isinstance(media, string_types):
        ...
        elif hasattr(media, 'entity_type') and media.entity_type in (
            'FileComponent',
        ):
            ...
            raise ValueError(
                'Unable to encode media of type: {0}'.format(
                    type(media)
                )
            )
    ...
    def get_upload_metadata(
        ...
        *file_name* and *file_size* should match the components details.
        ...
        try:
            ...
            # Raise informative error if the action is not supported.
            if (
                'Invalid action u\'get_upload_metadata\''
                in error.message
            ):
                raise ftrack_api.exception.ServerCompatibilityError(
    ...
    def send_review_session_invites(self, invitees):
        ...
        *invitee* is a list of ReviewSessionInvitee objects.
        ...
        try:
            ...
            # Raise informative error if the action is not supported.
            if (
                'Invalid action u\'send_review_session_invite\''
                in error.message
            ):
                raise ftrack_api.exception.ServerCompatibilityError(
                    'Server version {0!r} does not support '
...
class OperationRecordingContext(object):
    ...
    def __enter__(self):
        '''Enter context.'''
        self._current_record_operations = (
            self._session.record_operations
        )
